...
class ReportMain(Base):
    ...
    eps = Column(Float(53))  # 每股收益
    eps_yoy = Column(Float(53))  # 每股收益同比(%)
    bvps = Column(Float(53))  # 每股净资产
    roe = Column(Float(53))  # 净资产收益率(%)
    epcf = Column(Float(53))  # 每股现金流量(元)
    net_profits = Column(Float(53))  # 净利润(万元)
    profits_yoy = Column(Float(53))  # 净利润同比(%)
    distrib = Column(Float(53))  # 分配方案
    net_profit_ratio = Column(Float(53))  # 净利率(%)
    gross_profit_rate = Column(Float(53))  # 毛利率(%)
    business_income = Column(Float(53))  # 营业收入(百万元)
    bips = Column(Float(53))  # 每股主营业务收入(元)
    arturnover = Column(Float(53))  # 应收账款周转率(次)
    arturndays = Column(Float(53))  # 应收账款周转天数(天)
    inventory_turnover = Column(Float(53))  # 存货周转率(次)
    inventory_days = Column(Float(53))  # 存货周转天数(天)
    currentasset_turnover = Column(Float(53))  # 流动资产周转率(次)
    currentasset_days = Column(Float(53))  # 流动资产周转天数(天)
    mbrg = Column(Float(53))  # 主营业务收入增长率(%)
    nprg = Column(Float(53))  # 净利润增长率(%)
    nav = Column(Float(53))  # 净资产增长率
    targ = Column(Float(53))  # 总资产增长率
    epsg = Column(Float(53))  # 每股收益增长率
    seg = Column(Float(53))  # 股东权益增长率
    currentratio = Column(Float(53))  # 流动比率
    quickratio = Column(Float(53))  # 速动比率
    cashratio = Column(Float(53))  # 现金比率
    icratio = Column(Float(53))  # 利息支付倍数
    sheqratio = Column(Float(53))  # 股东权益比率
    adratio = Column(Float(53))  # 股东权益增长率
    cf_sales = Column(Float(53))  # 经营现金净流量对销售收入比率
    rateofreturn = Column(Float(53))  # 资产的经营现金流量回报率
    cf_nm = Column(Float(53))  # 经营现金净流量与净利润的比率
    cf_liabilities = Column(Float(53))  # 经营现金净流量对负债比率
    cashflowratio = Column(Float(53))  # 现金流量比率
